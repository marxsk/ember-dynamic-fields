...
moduleForComponent(
  "dynamic-fields",
  "Integration | Component | dynamic fields",
  {
    integration: true
  }
);
...
test("*it yields content according to number of records from dataObject", function(assert) {
  ...
  assert.equal(
    input1.value,
    "first",
    "The first input box contains right string"
  );
  assert.equal(
    3,
    this.$("input").length,
    "There are two filled input boxes and one empty"
  );
});
...
test("it adds new line when last one is not empty", async function(assert) {
  ...
  assert.equal(
    2,
    this.$("input").length,
    "There are two input boxes because first one contains text"
  );
  ...
  assert.equal(
    2,
    this.$("input").length,
    "There are still two input boxes because only first one contains text"
  );
  ...
  assert.equal(
    3,
    this.$("input").length,
    "There are three input boxes because second one contains text"
  );

  assert.equal(
    "new-first",
    this.get("data")[0],
    "First element in the result is okay"
  );
  assert.equal(
    "second",
    this.get("data")[1],
    "Second element in the result is okay"
  );
});
...
test('it adds new line when last one is not empty and there is less than "max" lines', async function(assert) {
  ...
  assert.equal(
    2,
    this.$("input").length,
    'There are still just two input boxes because of "max" limit'
  );
...
test("it adds new line when last one is not empty (with dataObjectKey)", async function(assert) {
  ...
  assert.equal(
    2,
    this.$("input").length,
    "There are still two input boxes because first one contains text"
  );
...
test("it removes line when it is empty and it is not last one", async function(assert) {
  ...
  assert.equal(
    1,
    this.$("input").length,
    "There is just one input box because first one was empty"
  );
});
...
test("it works with ember-power-select", async function(assert) {
  ...
  assert.equal(
    ...
    2,
    "Two ember-power-select are rendered"
  );
});
...
test("it works with ember-power-select-multiple", async function(assert) {
  ...
  assert.deepEqual(
    this.get("data.firstObject"),
    ["abc", "xyz"],
    "There are two selected items in the first field"
  );
  assert.equal(
    Ember.$(".ember-power-select-trigger").length,
    2,
    "Two ember-power-select-multiple are rendered"
  );
});
