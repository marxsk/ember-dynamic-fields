...
export default Ember.Component.extend({
  ...
  init() {
    this._super();
    Ember.Logger.assert(
      this.get("dataObject") !== undefined,
      "dynamic-fields: dataObject have to be defined"
    );
  },
  ...
  didReceiveAttrs() {
    ...
    if (Ember.isArray(x)) {
      ...
      Ember.Logger.assert(
        "Input object have to be Ember.Array if you want to pass data in"
      );
    }
  },
  ...
  actions: {
    ...
    update: function(target, targetIndex, arg3) {
      ...
      if (this.get("_source") === undefined) {
        Ember.Logger.debug(
          "dynamic-fields: Source was undefined, creating the array with empty item"
        );
        this.set("_source", Ember.A());

        // @todo-note: if dataObjectKey is undefined then use dataObject directly
        if (this.get("dataObjectKey")) {
          this.get("dataObject").set(
            this.get("dataObjectKey"),
            this.get("_source")
          );
        } else {
